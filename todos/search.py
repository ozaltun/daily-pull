...
email= os.getenv('MYEMAIL')+'@gmail.com'
...
def addArtist2JSON(ArtistName, id=None):
    ArtistName, artistID, artistKeys= get_MusicBrainz(id, ArtistName)
    artistList = {artistID: artistKeys}
    artist2ID = {ArtistName: artistID}

    try:
        with open('data/artistID.json', mode='r') as feedsjson:
            feeds = json.load(feedsjson)

    except ValueError:
        with open('data/artistID.json', mode='w') as feedsjson:
    ...
    try:
        with open('data/artistList.json', mode='r') as feedsjson:
            feeds = json.load(feedsjson)
    except ValueError:
        with open('data/artistList.json', mode='w') as feedsjson:
            json.dump(dict(), feedsjson)
        with open('data/artistList.json', mode='r') as feedsjson:
            feeds = json.load(feedsjson)
    with open('data/artistList.json', mode='w') as feedsjson:
...
def removeArtistFromJSON(ArtistName, id=None):
    try:
        with open('data/artistID.json', mode='r') as feedsjson:
            feeds = json.load(feedsjson)
    except ValueError:
